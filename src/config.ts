import { isSummaryTheme, type SummaryTheme } from './helpers.js';

export const parseSetupTerraform = (value: string | undefined): 'true' | 'false' => {
  if (value === undefined || value === '') return 'true';
  if (value === 'true' || value === 'false') return value;
  throw new Error('Invalid setup-terraform input: expected true or false.');
};

export const parseSummaryTheme = (value: string | undefined): SummaryTheme => {
  if (value === undefined || value === '') return 'default';
  if (isSummaryTheme(value)) return value;
  throw new Error('Invalid summary-theme input: expected default, colorblind, or minimal.');
};

export const parsePlanExitCode = (value: string | undefined): '0' | '1' | '2' => {
