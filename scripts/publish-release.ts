...
// Only locally constructed diagnostics may reach the CLI log. Never wrap
// request errors or response bodies in this type: they may contain credentials.
class PublicationError extends Error {}

export const releaseNotes = (changelog: string, tag: string): string => {
  if (!/^v\d+\.\d+\.\d+$/u.test(tag)) {
    throw new PublicationError('Expected a version tag such as v2.0.0');
  }
  const headers = [...changelog.matchAll(/^## \[([^\]]+)\] - \d{4}-\d{2}-\d{2}[ \t]*\r?$/gm)]
    .filter((match) => match[1] === tag.slice(1));
  const header = headers[0];
  if (headers.length !== 1 || !header) {
    throw new PublicationError(`Expected exactly one dated CHANGELOG.md section for ${tag}`);
  }
  const remainder = changelog.slice(header.index + header[0].length);
  const notes = remainder.split(/^## |^\[[^\]]+\]:\s+https?:\/\//m, 1)[0]?.trim();
  if (!notes) {
    throw new PublicationError(`Release notes for ${tag} are empty`);
  }
  return notes;
};
...
const object = (value: unknown): Record<string, unknown> => {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new PublicationError('Malformed GitHub API response');
  }
...
const parseRelease = (value: unknown): Release => {
  ...
  if (typeof release.id !== 'number' || !Number.isSafeInteger(release.id) || release.id <= 0 ||
    ...
    throw new PublicationError('Malformed GitHub release metadata');
  }
...
export const publishRelease = async (input: Publication): Promise<'published' | 'already-published'> => {
  ...
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repository) || !/^v\d+\.\d+\.\d+$/u.test(tag) ||
    ...
    throw new PublicationError('Publication requires owner/repo, a version tag, exact commit and tag-object SHAs, notes, and a token');
  }
  ...
  const request = async (endpoint: string, method = 'GET', body?: Record<string, unknown>): Promise<unknown> => {
    const response = await fetch(`https://api.github.com${base}${endpoint}`, {
      ...
      // Keep the operation, never the underlying error's headers or message.
      throw new PublicationError(
        `GitHub ${method} ${endpoint} failed before receiving a response; check connectivity or timeout, then rerun`
      );
    });
    if (!response.ok) {
      throw new PublicationError(`GitHub ${method} ${endpoint} failed with HTTP ${String(response.status)}; rerun after resolving the error`);
    }
    return response.json().catch(() => {
      throw new PublicationError(`GitHub ${method} ${endpoint} response could not be read as JSON; check GitHub availability, then rerun`);
    });
  };
  const checkTag = async (): Promise<void> => {
    ...
    if (ref.ref !== `refs/tags/${tag}` || target.type !== 'tag' || target.sha !== tagObject) {
      throw new PublicationError('Remote tag no longer matches the verified signed tag object');
    }
  };
  const checkIdentity = (release: Release): void => {
    if (release.tag_name !== tag || release.prerelease) {
      throw new PublicationError('Conflicting release tag or prerelease state');
    }
  };
  ...
  for (let page = 1; ; page += 1) {
    ...
    if (!Array.isArray(releases)) {
      throw new PublicationError('Expected a GitHub release list');
    }
  ...
  if (matches.length > 1) {
    throw new PublicationError(`Multiple releases exist for ${tag}; reconcile them before retrying`);
  }
  ...
  if (release.draft) {
    if (release.immutable || release.target_commitish !== sha || release.name !== tag ||
      ...
      throw new PublicationError('Existing draft differs from the verified commit, changelog, title, or expected empty asset list');
    }
    await checkTag();
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (current.id !== release.id) throw new PublicationError('GitHub returned a different release ID');
  ...
  throw new PublicationError(`Release ${tag} is not published and immutable; keep the major tag unchanged`);
};

export const runPublication = async (): Promise<number> => {
  try {
    ...
    try {
      ...
      throw new PublicationError('Cannot read CHANGELOG.md; check the working directory and file permissions, then rerun');
    }
    ...
    console.error(error instanceof PublicationError ? error.message :
      'Release publication failed unexpectedly; check changelog access and GitHub connectivity, then rerun. Keep the major tag unchanged.');
    return 1;
  }
};

if (import.meta.main) process.exitCode = await runPublication();
